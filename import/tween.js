...
(function () {
  ...
  Date.now = (Date.now || function () {return new Date().getTime();}); // IE 8
...
TWEEN.Tween = function (object) {
  ...
  for (var field in object) { // Set all starting values present on the target object
  ...
  this.start = function (time) {
    ...
    for (var property in _valuesEnd) {
      if (_valuesEnd[property] instanceof Array) { // Check if an Array was provided as property value
        if (_valuesEnd[property].length === 0) {continue;}
        _valuesEnd[property] = [_object[property]].concat(_valuesEnd[property]); // Create a local copy of the Array with the start value at the front
      }
      _valuesStart[property] = _object[property];
      if ((_valuesStart[property] instanceof Array) === false) {_valuesStart[property] *= 1.0;} // Ensures we're using numbers, not strings
  ...
  this.update = function (time) {
    ...
    for (property in _valuesEnd) {
      ...
      if (end instanceof Array) {
        ...
        if (typeof (end) === 'string') { // Parses relative end values with start as base (e.g.: +10, -3)
          end = start + parseFloat(end, 10);
        }
        if (typeof (end) === 'number') { // Protect against non numeric properties.
    ...
    if (elapsed === 1) {
      if (_repeat > 0) {
        ...
        for (property in _valuesStartRepeat) { // Reassign starting values, restart by making startTime = now
        ...
        } // Make chained tweens start when they should, even if update() was called past the tween duration
        return false;
      }
    }
    return true;
...
TWEEN.Easing = {
  ...
  Bounce: {
    In: function (k) {return 1 - TWEEN.Easing.Bounce.Out(1 - k);},
    Out: function (k) {
...
TWEEN.Interpolation = {
  ...
  Bezier: function (v, k) {
    var b = 0, n = v.length - 1, pw = Math.pow, bn = TWEEN.Interpolation.Utils.Bernstein;
...
(function (root) { // UMD (Universal Module Definition)
  if (typeof define === 'function' && define.amd) {define([], function () {return TWEEN;}); // AMD
  } else if (typeof exports === 'object') {module.exports = TWEEN; // Node.js
  } else if (root !== undefined) {root.TWEEN = TWEEN;} // Global variable
})(this);
